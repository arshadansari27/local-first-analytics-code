#!/usr/bin/env python3
"""
Dagster Asset-Based Pipeline - Level 3 Orchestration

Demonstrates:
- Asset-centric thinking (data products, not tasks)
...
def raw_events(context: AssetExecutionContext) -> pl.DataFrame:
    ...
    response = httpx.get("https://api.example.com/events", timeout=30.0)
    data = response.json()
